...
const NewsItem: React.FC<NewsItemProps> = ({ image, title, description, id }) => (
  <div className="w-full flex flex-col xl:flex-row gap-3 rounded-xl">
    <Link href={`/home/activity/${id}`} className="flex-shrink-0 w-full xl:w-1/3">
      <div className="  rounded-lg aspect-w-4 aspect-h-3">
        <Image
          src={image}
          alt={title}
          width={1000}
...
const Part4 = async () => {
  ...
  return (
    <div className="bg-[#222222] py-10 px-10 h-full 2xl:px-[250px]">
      <h2 className="text-white text-[28px] sm:text-[35px] font-[700] text-nowrap lg:mb-7 xl:mb-0 ">
      ...
      <div className="flex flex-col w-full lg:flex-row gap-8">
        ...
        <div className="flex flex-col lg:w-7/12 gap-7 2xl:gap-36 lg:mt-[33px] xl:mt-[80px] 2xl:mt-[40px]">
          {data?.data?.slice(1).map((news: News, index: number) => (
            <NewsItem
              ...
              title={news.title}
              // description={truncate("lorem30sfkmsdlfl;ksjdlkfjlksjdlkfj;lskjd;lkjf;lkjs;lkdfj;lksjd;lkfj;lskjdlfkj;lskdjfljs;lkdjf;ljs;ldkjf; lkjs;dljf;lksjd;lkfj;lskdjl;fj;lsdj;flksl;dfs;djflksdkljf;lskdjf fjlsj;lfj;lksadfsadfasdfsf")}
              description={truncate(news?.dec)}
            />
          ))}
        </div>
      </div>
